...
import qualified RedeemerFtPolicy  as OnChain 
...
runDeploy baseFolder uTxOutRefStr = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'redeemerFtPolicy' Script..."
        let 
            policy = OnChain.redeemerFtPolicy 
            policy_CS = OffChainHelpers.getCurSymbolOfPolicy policy
        DeployHelpers.deployMintingPolicy (path SystemFilePathPosix.</> folderName) "redeemerFtPolicy" policy policy_CS
    ------------------------------
    return ()
