...
import qualified AllwaysTruePolicy  as OnChain 
...
runDeploy baseFolder = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'allwaysTruePolicy' Script..."
        let 
            policy = OnChain.allwaysTruePolicy
            policy_CS = OffChainHelpers.getCurSymbolOfPolicy policy
        DeployHelpers.deployMintingPolicy (path SystemFilePathPosix.</> folderName) "allwaysTruePolicy" policy policy_CS
    ------------------------------
    return ()
