...
import qualified AllwaysFalsePolicy  as OnChain 
...
runDeploy baseFolder = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'allwaysFalsePolicy' Script..."
        let 
            policy = OnChain.allwaysFalsePolicy
            policy_CS = OffChainHelpers.getCurSymbolOfPolicy policy
        DeployHelpers.deployMintingPolicy (path SystemFilePathPosix.</> folderName) "allwaysFalsePolicy" policy policy_CS
    ------------------------------
    return ()
