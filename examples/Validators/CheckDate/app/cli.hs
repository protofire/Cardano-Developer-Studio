...
menuLoop :: P.IO ()
menuLoop = do
    P.putStrLn "Choose an option:"
    P.putStrLn "1. Deploy Validators"
    P.putStrLn "2. Create and Submit Transactions"
    P.putStrLn "3. Exit"
    option <- P.getLine
    case option of
        "1" -> do
            ...
            menuLoop
        "2" -> do
            createAndSubmitTransactions
            menuLoop
        "3" -> P.putStrLn "Exiting..."
        _   -> do
...
createAndSubmitTransactions :: P.IO ()
createAndSubmitTransactions = do
    SystemProcess.callCommand "bash examples/Validators/CheckDate/scripts/run.sh"
