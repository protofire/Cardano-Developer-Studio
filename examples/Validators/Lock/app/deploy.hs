...
import           LockValidator as OnChain (lockValidator)
...
deploy = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'lockValidator' Script..."
        let validator = OnChain.lockValidator
            validator_Hash = OffChainHelpers.hashValidator validator
            validator_Address = OffChainHelpers.addressValidator validator_Hash
        _ <- DeployHelpers.deployValidator (path SystemFilePathPosix.</> folderName) "lockValidator" validator
        _ <- DeployHelpers.deployValidatorHash (path SystemFilePathPosix.</> folderName) "lockValidator" validator_Hash
        DeployHelpers.deployValidatorAddress (path SystemFilePathPosix.</> folderName) "lockValidator" validator_Address
    ------------------------------
    return ()
