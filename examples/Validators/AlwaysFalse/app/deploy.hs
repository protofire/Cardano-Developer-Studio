...
import           AlwaysFalseValidator as OnChain (alwaysFalseValidator)
...
runDeploy baseFolder = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'alwaysFalseValidator' Script..."
        let validator = OnChain.alwaysFalseValidator
            validator_Hash = OffChainHelpers.hashValidator validator
            validator_Address = OffChainHelpers.addressValidator validator_Hash
        _ <- DeployHelpers.deployValidator (path SystemFilePathPosix.</> folderName) "alwaysFalseValidator" validator
        _ <- DeployHelpers.deployValidatorHash (path SystemFilePathPosix.</> folderName) "alwaysFalseValidator" validator_Hash
        DeployHelpers.deployValidatorAddress (path SystemFilePathPosix.</> folderName) "alwaysFalseValidator" validator_Address
    ------------------------------
    return ()
