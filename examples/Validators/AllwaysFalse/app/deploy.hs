...
import           AllwaysFalseValidator as OnChain (allwaysFalseValidator)
...
deploy = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'allwaysFalseValidator' Script..."
        let validator = OnChain.allwaysFalseValidator
            validator_Hash = OffChainHelpers.hashValidator validator
            validator_Address = OffChainHelpers.addressValidator validator_Hash
        _ <- DeployHelpers.deployValidator (path SystemFilePathPosix.</> folderName) "allwaysFalseValidator" validator
        _ <- DeployHelpers.deployValidatorHash (path SystemFilePathPosix.</> folderName) "allwaysFalseValidator" validator_Hash
        DeployHelpers.deployValidatorAddress (path SystemFilePathPosix.</> folderName) "allwaysFalseValidator" validator_Address
    ------------------------------
    return ()
