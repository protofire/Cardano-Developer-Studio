...
import           FreeValidator as OnChain (freeValidator)
...
deploy = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'freeValidator' Script..."
        let validator = OnChain.freeValidator
            validator_Hash = OffChainHelpers.hashValidator validator
            validator_Address = OffChainHelpers.addressValidator validator_Hash
        _ <- DeployHelpers.deployValidator (path SystemFilePathPosix.</> folderName) "freeValidator" validator
        _ <- DeployHelpers.deployValidatorHash (path SystemFilePathPosix.</> folderName) "freeValidator" validator_Hash
        DeployHelpers.deployValidatorAddress (path SystemFilePathPosix.</> folderName) "freeValidator" validator_Address
    ------------------------------
    return ()
