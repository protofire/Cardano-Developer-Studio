...
import           AllwaysTrueValidator as OnChain (allwaysTrueValidator)
...
deploy = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'allwaysTrueValidator' Script..."
        let validator = OnChain.allwaysTrueValidator
            validator_Hash = OffChainHelpers.hashValidator validator
            validator_Address = OffChainHelpers.addressValidator validator_Hash
        _ <- DeployHelpers.deployValidator (path SystemFilePathPosix.</> folderName) "allwaysTrueValidator" validator
        _ <- DeployHelpers.deployValidatorHash (path SystemFilePathPosix.</> folderName) "allwaysTrueValidator" validator_Hash
        DeployHelpers.deployValidatorAddress (path SystemFilePathPosix.</> folderName) "allwaysTrueValidator" validator_Address
    ------------------------------
    return ()
