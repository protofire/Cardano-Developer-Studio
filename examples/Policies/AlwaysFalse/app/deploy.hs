...
import qualified AlwaysFalsePolicy  as OnChain 
...
runDeploy baseFolder = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'alwaysFalsePolicy' Script..."
        let 
            policy = OnChain.alwaysFalsePolicy
            policy_CS = OffChainHelpers.getCurSymbolOfPolicy policy
        DeployHelpers.deployMintingPolicy (path SystemFilePathPosix.</> folderName) "alwaysFalsePolicy" policy policy_CS
    ------------------------------
    return ()
