...
import qualified AlwaysTruePolicy  as OnChain 
...
runDeploy baseFolder = do
    ...
    ------------------------------
    do 
        P.putStrLn "Generating 'alwaysTruePolicy' Script..."
        let 
            policy = OnChain.alwaysTruePolicy
            policy_CS = OffChainHelpers.getCurSymbolOfPolicy policy
        DeployHelpers.deployMintingPolicy (path SystemFilePathPosix.</> folderName) "alwaysTruePolicy" policy policy_CS
    ------------------------------
    return ()
